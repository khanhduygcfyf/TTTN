    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_profile"])) {
        ...
        if(empty(array_filter($error))) {
            ...
            try {
                ...
                // Đảm bảo $_SESSION['cart_total'] tồn tại
                $total_price = isset($_SESSION['cart_total']) ? $_SESSION['cart_total'] : 0;
                $points_to_add = floor($total_price / 100) * 2; // Cứ 100 tiền = 2 điểm

                // Lấy điểm hiện tại của người dùng
                $current_points = $CustomerModel->get_user_points($user_id);

                // Cập nhật điểm
                $new_points = $current_points + $points_to_add;
                $CustomerModel->update_user_points($user_id, $new_points);

                // Cập nhật cấp bậc dựa trên điểm
                $new_level = $CustomerModel->get_user_level($new_points);
                $CustomerModel->update_user_level($user_id, $new_level);

                // Cập nhật lại SESSION
                $_SESSION['user']['full_name'] = $full_name;
                $_SESSION['user']['phone'] = $phone;
                $_SESSION['user']['address'] = $address;
                $_SESSION['user']['points'] = $new_points; // Cập nhật điểm mới
                $_SESSION['user']['level'] = $new_level;   // Cập nhật cấp bậc mới
                if (!empty($image)) {
...
if (isset($_SESSION['user']['id'])) {
    ...
    $user_id = $_SESSION['user']['id'];
    $user = $CustomerModel->select_email_in_users($_SESSION['user']['email']); // Lấy thông tin đầy đủ từ cơ sở dữ liệu
