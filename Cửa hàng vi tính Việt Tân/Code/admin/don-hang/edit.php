...
<div class="container pt-4">
    <article class="card">
        ...
        <div class="card-body mt-2">
            ...
            <!-- Add the evaluation form here -->
            <div class="row">
                <div class="col-lg-12">
                    <div class="card mb-4 bg-custom" style="background-color: #ffff;">
                        <div class="card-body text-dark">
                            <h6 class="mb-4">Đánh giá sản phẩm</h6>
                            <?php if(isset($_SESSION['user'])) { ?>
                                <form action="index.php?url=submit-evaluation" method="post">
                                    <input type="hidden" name="product_id" value="<?=$product_id?>">
                                    <input type="hidden" name="user_id" value="<?=$_SESSION['user']['id']?>">
                                    <div class="form-floating mb-3">
                                        <select name="rating" class="form-select" id="floatingSelectRating" aria-label="Floating label select example" required>
                                        ...
                                        <label for="floatingSelectRating">Đánh giá</label>
                                    </div>
                                    <div class="form-floating mb-3">
                                        <textarea name="comment" class="form-control" id="floatingTextarea" rows="4" required></textarea>
                                        <label for="floatingTextarea">Bình luận</label>
                                    </div>
                                    <button type="submit" class="btn btn-custom">Gửi đánh giá</button>
                                </form>
                            <?php } else { ?>
                                <p>Vui lòng <a href="index.php?url=login">đăng nhập</a> để gửi đánh giá.</p>
                            <?php } ?>
                        </div>
                    </div>
                </div>
